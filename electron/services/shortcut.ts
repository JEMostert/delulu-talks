...
function shortcutDescription(shortcuts: unknown): string | undefined {
  ...
  const properties = entries.find(
    (entry) => Array.isArray(entry) && entry[0] === SHORTCUT_ID,
  )?.[1];
  const description = properties?.trigger_description;
  return description instanceof Variant
...
export class ShortcutService {
  ...
  private registerNative(accelerator: string): void {
    ...
    if (
      ...
      this.update({
        accelerator,
        registered: false,
        message: "Shortcut unavailable — choose another combination",
      });
  ...
  private async registerPortal(accelerator: string): Promise<void> {
    ...
    this.update({
      accelerator,
      registered: false,
      message: "Registering with the desktop shortcut portal",
    });
    const busAddress = compatibleSessionBusAddress(process.env);
    const bus = sessionBus(
      busAddress ? { busAddress } : undefined,
    ) as MessageBus & { name: string | null };
    this.bus = bus;
    bus.on("error", (error) =>
      this.update({
        registered: false,
        message: `Shortcut portal error: ${concise(error)}`,
      }),
    );
    try {
      ...
      try {
        ...
        /* Packaged apps already have a portal identity. */
      }
      const shortcuts = object.getInterface(
        "org.freedesktop.portal.GlobalShortcuts",
      ) as PortalInterface;
      this.shortcuts = shortcuts;
      shortcuts.on("Activated", (session: string, id: string) => {
        if (session === this.session && id === SHORTCUT_ID) this.press();
      });
      shortcuts.on("Deactivated", (session: string, id: string) => {
        if (session === this.session && id === SHORTCUT_ID) this.release();
      });
      shortcuts.on("ShortcutsChanged", (session: string, entries: unknown) => {
        ...
        if (description)
          this.update({
            ...
            message: `Wayland shortcut ready · ${description}`,
          });
      });
      ...
      if (createCode !== 0)
        throw new Error(
          createCode === 1
            ? "Shortcut permission was cancelled"
            : "The shortcut portal could not create a session",
        );
      this.session = String(createResults.session_handle?.value ?? "");
      if (!this.session)
        throw new Error("The shortcut portal returned no session");

      const [bindCode, bindResults] = await portalRequest(
        bus,
        `${token}_bind`,
        (handleToken) =>
            ...
            [
              [
                SHORTCUT_ID,
                {
                  ...
                  preferred_trigger: new Variant(
                    "s",
                    portalTrigger(accelerator),
                  ),
                },
              ],
            ],
      ...
      if (bindCode !== 0)
        throw new Error(
          bindCode === 1
            ? "Shortcut permission was cancelled"
            : "The shortcut portal rejected the binding",
        );
      const description =
        shortcutDescription(bindResults.shortcuts) ?? accelerator;
      this.update({
        accelerator: description,
        registered: true,
        message: `Wayland shortcut ready · ${description}`,
      ...
      this.update({
        accelerator,
        registered: false,
        message: `Shortcut unavailable: ${concise(error)}`,
      });
      throw error;
    }
  }
  ...
  async configure(): Promise<void> {
    if (!this.portal || !this.shortcuts || !this.session)
      throw new Error("The system shortcut editor is unavailable");
  ...
  private async closePortal(): Promise<void> {
    ...
    if (bus && session) {
      try {
        ...
        /* Disconnecting also destroys the session. */
      }
    }
    bus?.disconnect();
  }
