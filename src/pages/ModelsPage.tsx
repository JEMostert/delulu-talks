...
export function ModelsPage({ selected, saving, onSelect, onSetup }: { selected: ModelId; saving: boolean; onSelect: (id: ModelId) => void; onSetup: () => void }) {
  return (
    <div className="content-stack">
      <section className="page-intro">
        <div><span className="hello-pill"><Sparkles /> Three specialists, zero compromises</span><h2>Pick the brain for the job.</h2><p>Every model runs locally. Choose rich meeting notes, top-tier accuracy, or ultra-fast dictation.</p></div>
        <button className="primary-button" onClick={onSetup}><Download /> Set up selected model</button>
      </section>
        ...
        {MODELS.map((model, index) => {
          ...
          return (
            <article key={model.id} className={`model-card ${model.accent} ${active ? "selected" : ""}`}>
              ...
              <div className="model-specs"><span><Cpu />{model.size}</span><span><Languages />{model.languages}</span><span>{model.id === "nemotron35Streaming" ? <Radio /> : <Zap />}{model.latency}</span></div>
              <div className="mini-tags">{model.badges.map((badge) => <span key={badge}>{badge}</span>)}</div>
              <button disabled={active || saving} className={active ? "model-button active" : "model-button"} onClick={() => onSelect(model.id)}>{active ? "Currently selected" : <>Use this model <ChevronRight /></>}</button>
            </article>
          );
        })}
      </div>
