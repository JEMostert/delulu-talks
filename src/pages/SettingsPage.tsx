  ...
  return (
    <div className="content-stack settings-page">
      ...
      <section className="settings-group paper-card"><div className="group-heading"><p className="eyebrow">OUTPUT</p><h3>How your words land</h3></div>
        <SettingRow icon={SlidersHorizontal} title="Language" description="Cohere needs a language; MOSS and Nemotron can detect it."><select value={draft.language} onChange={(event) => setDraft({ ...draft, language: event.target.value })}>{LANGUAGES.map(([code, label]) => <option key={code} value={code}>{label}</option>)}</select></SettingRow>
